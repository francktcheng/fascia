        ...
        bool do_outerloop, bool calc_auto, bool verbose)
{
    ...
    double elt_total = 0.0;
    //record decomposed time
    double elt = 0.0;
    ...
    if (timing || verbose) {
        elt = timer() - elt;
        printf("Read in Data Time:\n\t%9.6lf seconds\n", elt);
    }

    double full_count = 0.0;  
    if (do_outerloop)
    {
        // parallelize the outerloop by omp
...
#pragma omp parallel reduction(+:full_count)
        ...
        if (timing || verbose) {
            elt = timer() - elt;
            printf("Computation Time OuterParal Mode:\n\t%9.6lf seconds\n", elt);
        }
    ...
    {
        // parallelize the innerloop by omp
        ...
        if (timing || verbose) {
            elt = timer() - elt;
            printf("Computation Time InnerParal Mode:\n\t%9.6lf seconds\n", elt);
        }
...
/**
 * @brief Run a batch of template files
 *
 * @param graph_file
 * @param batch_file
 * @param labeled
